...
This is the coordinator kit's original install path: a paste-able prompt (or manual steps) that
copies files directly into your project, instead of installing a plugin. It's for an existing
installation, or for anyone who prefers plain copied files over a plugin. See `README.md` for the
newer, primary plugin path, and `UPDATING.md` for updating an existing file-copy install.

The original install path: paste a prompt (or follow steps by hand) that copies `CLAUDE.md`,
`PROCESS.md`, `STATE.md`, `codex-setup.md` into a project, `memory-seed/*` into a Claude Code
memory directory, and `telegram-bridge/` outside the project. Still fully supported — kept for
existing installs and anyone who prefers a plain file copy over a plugin. An already-installed
project updates via `UPDATING.md`'s per-file rules, not by repeating the steps below — see
"Updating" below.
...
8. (Optional but recommended) Copy `memory-seed/*.md` into the new project's Claude Code memory
   ...
   re-learn them the hard way (each one below was originally a real correction after a real
   mistake). `CLAUDE.md` alone carries the rules for any session that loads it, so this step is
   not load-bearing — but only `memory-seed/MEMORY.md` (not the individual linked files) is
   guaranteed to auto-load into a fresh session's context the way project instructions do. That's
   why each line in `MEMORY.md` is written to be actionable on its own, not just a pointer — treat
   the linked per-topic files as reference detail you or an agent can open on demand, not as
   content you can assume is already in context. In the copied `MEMORY.md`, replace `<PROJECT>` in
...
**Why three sessions, not one:** same reasoning as "Update the plugin" above — `CLAUDE.md` is
both the file being rewritten and the running session's own operating instructions, loaded once
at session start, so the session that just wrote the new file can never be the one that starts
running under it.
